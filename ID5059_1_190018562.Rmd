...
Now for numeric variables, we use the impute function to change NAs to the mean. 

```{r}
vehicles$year[is.na(vehicles$year)] <- median(vehicles$year, na.rm = T)
vehicles$odometer[is.na(vehicles$odometer)] <- median(vehicles$odometer, na.rm = T)
```
...
```
Before modelling, lets choose covariates by comparing using machine learning feature selection;
...
# train the model
model <- train(price~., data=trainInd, method="lvq", preProcess="scale", trControl=control)
...
```{r}
vehicles_lm <- lm(price ~ year + manufacturer + transmission + state, data = vehicles, na.action = na.exclude)

predict_price <- predict(vehicles_lm, price)
```
